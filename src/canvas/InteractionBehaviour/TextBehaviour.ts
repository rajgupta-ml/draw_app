...
import {
  AddShapeCommand,
  RemoveShapeCommand,
} from "../UndoAndRedoCmd/ShapeCommand";
...
export class TextBehaviour implements IInteractionBehavior {
  ...
  onMouseDown({
    ...
    if (this.input === null) {
      ...
      for (let i = shapes.length - 1; i >= 0; i--) {
        shape = shapes[i]!;
        if (
          shape.type === TOOLS_NAME.TEXT &&
          this.shapeRender.isPointInShape(shape as TextShape, x, y)
        ) {
      ...
      if (this.isEditing && shape) {
        executeCanvasCommnad(
          new RemoveShapeCommand(manager, shape, originalIndex),
        );
        drawCanvas();
        this.isEditing = false;
      }

      this.input = this.createInputBox(
        x,
        y,
        rawX,
        rawY,
  ...
  private finalizeCreateInput = (
    ...
    const fontSize =
      this.textToEdit?.config.fontSize ??
      parseInt(config.fontSize).toString() ??
      "16";
    const fontFamily =
      this.textToEdit?.config.fontFamily ?? config.fontFamily ?? "Arial";
