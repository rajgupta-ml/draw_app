    ...
    render(existingShape: RightArrowShape, canvas: RoughCanvas): void {
        ...
        if (effectiveShaftLength < 0) {
          ...
          if (length < arrowheadLength / 2) { 
            ...
            this._drawArrowPath(canvas, startX, startY, endX, endY, effectiveShaftLength, scaledArrowheadLength, scaledArrowheadWidth, angle, config);
          }
        } else {
          this._drawArrowPath(canvas, startX, startY, endX, endY, effectiveShaftLength, arrowheadLength, arrowheadWidth, angle, config);
        }
      }
      ...
      private _drawArrowPath(
        ...
        const basePoint1X = tipX - arrowheadLength * Math.cos(angle) + (arrowheadWidth / 2) * Math.sin(angle);
        const basePoint1Y = tipY - arrowheadLength * Math.sin(angle) - (arrowheadWidth / 2) * Math.cos(angle);
    
        const basePoint2X = tipX - arrowheadLength * Math.cos(angle) - (arrowheadWidth / 2) * Math.sin(angle);
        const basePoint2Y = tipY - arrowheadLength * Math.sin(angle) + (arrowheadWidth / 2) * Math.cos(angle);
        ...
        canvas.line(startX + offsetX, startY + offsetY, shaftEndX + offsetX, shaftEndY + offsetY, shapeConfig);
