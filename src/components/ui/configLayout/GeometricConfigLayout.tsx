    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center gap-2">
        {(resolvedTheme === "light" || resolvedTheme === "dark") &&
          backgroundColor[resolvedTheme].map((color, index) => (
            <div
              onClick={() => handleBackgroundColorChange(color)}
              key={index}
              className={cn(
                "flex h-6 w-6 cursor-pointer items-center justify-center rounded-md border border-gray-300 transition-all ease-in-out hover:scale-110 dark:border-gray-600",
                config.fill === color
      ...
      <div className="flex gap-3">
        {widths.map((width) => (
          <button
            key={width}
            className={cn(
              "flex h-8 w-8 cursor-pointer items-center justify-center rounded-md border border-gray-300 transition-colors dark:border-gray-600",
...
const StrokeStyleLayout = () => {
  ...
  const currentStyleKey =
    styles.find(
      (s) => JSON.stringify(s.dash) === JSON.stringify(config.strokeLineDash),
    )?.key ?? "solid";
...
const RoughnessSliderLayout = () => {
  ...
  const displayValue =
    config.roughness !== undefined
      ? Math.round((config.roughness / 2) * 100)
      : 100;
    ...
    <div className="flex w-full flex-col gap-2">
      ...
      <div className="flex items-center gap-3">
        <input
          ...
          className="accent-sidebar-accent dark:accent-sidebar-accent-dark h-1.5 w-full cursor-pointer rounded-lg" // Styled slider
        />
        <span className="w-8 text-right text-xs text-gray-700 dark:text-gray-300">
          {displayValue}%
        </span>
      </div>
    </div>
...
const FillLayout = () => {
  ...
  const handleFillStyleChange = (style: Options["fillStyle"]) => {
    handleConfigChange({
      ...
      fillStyle: style,
      fill:
        style !== "none" && config.fill === "transparent"
          ? config.stroke
          : config.fill,
    });
  };
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex gap-3">
          <button
            className={cn(
              "flex h-8 w-8 cursor-pointer items-center justify-center rounded-md border border-gray-300 transition-colors dark:border-gray-600",
              config.fillStyle === "hachure" && config.fill !== "transparent"
                ? "bg-sidebar-accent text-sidebar-accent-foreground shadow-md"
                : "hover:bg-gray-100",
            )}
          ...
          <button
            className={cn(
              ...
              config.fillStyle === "solid" && config.fill !== "transparent"
                ? "bg-sidebar-accent text-sidebar-accent-foreground shadow-md"
                : "hover:bg-gray-100 dark:hover:bg-gray-700",
            )}
